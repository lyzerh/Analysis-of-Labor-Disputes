const fs = require('fs');
const content = `import React, { useMemo, useState, useEffect } from 'react';
...
export const CaseAnalysisView: React.FC<CaseAnalysisViewProps> = ({ records: initialRecords }) => {
  ...
  const toggleSelect = (id: string) => {
    ...
    if (next.has(id)) {
      ...
      next.clear(); // Ensure only one is selected if it's a detail view, but the UI used checkboxes? Actually it used toggleSelect to add/remove. The original code supported multiple. Let's keep it supporting multiple.
      next.add(id);
    }
    setSelectedIds(next);
  };
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden p-6 lg:p-8 min-h-0">
            {selectedRecords.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400">
                ...
                <p>{filteredRecords.length === 0 ? '暂无可分析案例' : '请在左侧选择案例进行分析'}</p>
              </div>
            ) : (
...
`;
fs.writeFileSync('src/components/CaseAnalysisView.tsx', content);
console.log('Successfully updated CaseAnalysisView.tsx layout');
