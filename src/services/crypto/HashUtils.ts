/**
 * Hash contract shared by parser, Snapshot, SamplingRun, and AnalysisRun.
 *
 * Both runtime paths implement SHA-256 and therefore produce the same 64
 * hexadecimal characters. The pure TypeScript path is used when Web Crypto
 * is unavailable (for example, an insecure local browser context), so the
 * persisted provenance contract remains `sha256-v1` in every runtime.
 */
...
export async function hashString(value: string): Promise<string> {
  ...
  if (subtle && typeof subtle.digest === 'function') {
    try {
      ...
      // Fall through to the same SHA-256 implementation used in restricted runtimes.
    }
  }
