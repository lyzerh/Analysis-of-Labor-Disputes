...
Mapping policy for supplied claim targets:
- When a supplied judgment item clearly addresses a supplied target claim, resolve it directly as supported, partially_supported, or not_supported even when the wording does not repeat the canonical claim label. The target identity, judgment scope, and source evidence must still be clear.
- Global wording is not ambiguity by itself. If a judgment clearly dismisses all claims (for example, "all claims are dismissed" / "驳回全部诉讼请求") and the supplied targets are within that scope, resolve every target as not_supported and allow them to share the same judgmentItemId.
- If the judgment explicitly grants some supplied claims and dismisses the residual claims (for example, "other claims are dismissed" / "驳回其他诉讼请求"), resolve only the unsupported supplied targets that clearly fall within that residual scope as not_supported. If residual scope cannot be established for a target, keep that target unresolved.
- One judgment item may resolve multiple supplied claim targets. Return exactly one ClaimResolution per target; sharing a judgmentItemId is valid when the item clearly covers each target.
- For amount-based claims, compare amounts only after confirming that the claim amount and judgment amount refer to the same target matter: equal awarded amount means supported; a positive awarded amount lower than the requested amount means partially_supported; an explicit rejection or zero award means not_supported. Amount mismatch alone is not enough to establish the mapping.
- Do not mark a supplied target unresolved merely because the judgment is global, collective, residual, or uses a non-canonical claim label. Use unresolved only when scope is unclear, candidate items conflict, a required item or source evidence is missing, the amount cannot be tied to the same claim, required appellate context is absent, or supplied facts conflict.
