...
export type XaiConnectionStatus =
  | 'unknown'
  | 'testing'
  | 'available'
  | 'invalid_key'
...
const XAI_API_ROOT = `${XAI_API_BASE_URL}/chat/completions`;
const CONNECTION_TIMEOUT_MS = 10_000;
const CONNECTION_MAX_OUTPUT_TOKENS = 1_024;
...
const requestBody = (request: GeminiResultGenerateRequest, modelName: string): Record<string, unknown> => ({
  ...
  response_format: request.config?.responseJsonSchema
    ? {
      type: 'json_schema',
      json_schema: {
        name: 'semantic_resolution_result',
        strict: true,
...
export interface BrowserXaiSemanticClientOptions {
  caseId?: string;
  modelName?: string;
}

export const createBrowserXaiSemanticClient = (
  apiKey: string,
  options: BrowserXaiSemanticClientOptions = {},
): GeminiSemanticResultClient => ({
  models: {
    generateContent: async (request) => {
      traceConnection('before fetch');
      const response = await fetch(XAI_API_ROOT, {
        method: 'POST',
        headers: {
      ...
      if ((finishReason === 'length' || nativeFinishReason === 'length') && !text) {
        throw providerError(
          'xAI semantic output was truncated before JSON content was generated',
          response.status,
          'output_truncated',
        );
      }
...
const classifyStatus = (
  ...
  name?: string,
  code?: string,
): Exclude<XaiConnectionStatus, 'unknown' | 'testing' | 'available'> => {
...
export const testXaiConnection = async (apiKeyOverride?: string): Promise<XaiConnectionStatus> => {
  ...
  traceConnection(`apiKey prefix=${apiKey.trim().startsWith('xai-') ? 'xai-' : apiKey.trim() ? 'other' : 'none'}`);
  ...
  try {
    ...
    const client = createBrowserXaiSemanticClient(apiKey.trim());
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => {
          controller.abort();
          reject(Object.assign(new Error('xAI connection timeout'), { name: 'AbortError' }));
        }, CONNECTION_TIMEOUT_MS);
      }),
    ]);
    return response.text ? 'available' : 'unknown_error';
  } catch (error) {
    const candidate = error as { status?: number; message?: string; name?: string; stack?: string; code?: string };
    const status = classifyStatus(candidate.status, candidate.message || 'xAI connection failed', candidate.name, candidate.code);
