  ...
  public static async getPipelineHealthStats(): Promise<DataPipelineHealthStats> {
    ...
    const layers = [
      ...
      },
      {
        ...
        subText: `已入库 ${rawDocs.length} 份真实裁判文书`,
      ...
      },
      {
        ...
        subText: `成功解析 ${records.length} 个案例（失败 ${failedCount} 个案例）`,
      ...
      },
      {
        ...
        subText: `达标准入 ${qualifiedRecords.length} 个案例 | 待优化池 ${pendingReviewRecords.length} 个案例`,
      ...
      },
      {
        ...
        count: qualifiedRecords.length,
        subText: `已向分析引擎提供 ${qualifiedRecords.length} 个有效案例`,
        status: qualifiedRecords.length > 0 ? ('healthy' as const) : ('warning' as const),
        rateText: '100% 消费就绪',
      },
    ];
